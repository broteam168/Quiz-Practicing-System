            ...
            <div class="filter">
                ...
                <form action="lessons" method="GET" id="filterForm">
                    ...
                    <div class="filter__status">
                        ...
                        <input class="filter__status-select" type="number" min="1" id="order-input"
                               pattern="^[1-9]\d*$" inputmode="numeric" value="${param.order != 0 ? param.order: ""}"
                        ...
                        <div class="filter__status-select">
                            <select id="topic-select" class="select-field" name="topic_id" ${action eq "view" ? "disabled":""}>
                                ...
                                <c:forEach var="topic" items="${topics}">
                                    <option value="${topic.id}" ${topic_id == topic.id ? "selected" : ""}>
                                        ${topic.name}
                                    </option>
                                </c:forEach>
                            </select>
                    ...
                    <div class="filter__status">
                        ...
                        <div class="filter__status-select">
                            <select id="lesson_type" name="lesson_type">
                                <c:if test="${param.lesson_type == 0}">
                                    <option selected value=0>All</option>
                                </c:if>
                                <c:if test="${param.lesson_type != 0}">
                                    <option value=0>All</option>
                                </c:if>
                                <c:if test="${param.lesson_type == 1}">
                                    <option selected value=1>Topic</option>
                                </c:if>
                                <c:if test="${param.lesson_type != 1}">
                                    <option value=1>Topic</option>
                                </c:if>
                                <c:if test="${param.lesson_type == 2}">
                                    <option selected value=2>Lesson</option>
                                </c:if>
                                <c:if test="${param.lesson_type != 2}">
                                    <option value=2>Lesson</option>
                                </c:if>
                                <c:if test="${param.lesson_type == 3}">
