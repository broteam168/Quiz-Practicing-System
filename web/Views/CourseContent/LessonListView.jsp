<%@page contentType="text/html" pageEncoding="UTF-8" %>
    <%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
        ...
        <body>
            <jsp:include page="./LessonFilterPopup.jsp"></jsp:include>
            ...
            <div class="main">
                <div class="header__content">
                    <div class="main__title">
                        <div class="main__title-text">Subject Lessons</div>
                        <div class="main__title-text subject_info"><strong>Subject:</strong> ${subject.getTitle()}</div>
                        <div class="main__title-subtext">View and manage lessons</div>

                    </div>
                    <a class="back__btn btn" href="course-details?subject_id=${subject.subject_id}&action=view">Back To Subject </a>
                </div>

                <div class="main__navigation">

                    <div class="main__navigation-item ${lesson_type eq 0 ? " main__navigation-select" : "" }"
                        onclick="switchLessonType(0)">
                        ${lesson_type eq 0 ? "<div class='menu-select'></div>" : "" }
                        All Types
                    </div>

                    <div class="main__navigation-item ${lesson_type eq 1 ? " main__navigation-select" : "" }"
                        onclick="switchLessonType(1)">
                        ${lesson_type eq 1 ? "<div class='menu-select'></div>" : "" }
                        Topic
                    </div>

                    <div class="main__navigation-item ${lesson_type eq 2 ? " main__navigation-select" : "" }"
                        onclick="switchLessonType(2)">
                        ${lesson_type eq 2 ? "<div class='menu-select'></div>" : "" }
                        Lesson
                    </div>

                    <div class="main__navigation-item ${lesson_type eq 3 ? " main__navigation-select" : "" }"
                        onclick="switchLessonType(3)">
                        ${lesson_type eq 3 ? "<div class='menu-select'></div>" : "" }
                        Quiz
                    </div>
                ...
                <div class="main__data">
                    <div class="data__header">
                        <div class="data__header-title">
                            <!-- filter to get the element in topics that has same id with topic_id -->
                            <c:forEach var="topic" items="${topics}">
                                <c:if test="${topic.id == topic_id}">
                                    <c:set var="topic_name" value="${topic.name}" />
                                </c:if>
                            </c:forEach>
                            <!--  messages for filtering -->
                            <p>
                                Showing results for:<br/><br/>
                                ${param.name == null || param.name == "" ? "<span class='filter-text'>All</span>
                                lessons" : "Name '<span
                                    class='filter-text'>".concat(param.name).concat("</span>'")}
                                <!-- order -->
                    
                                ${param.order == null || param.order == 0 ? "" : "<br/><br/> Order
                                <span class='filter-text'>".concat(param.order).concat("</span>")}
                                <!-- topic -->
                                ${topic_name == null || param.topic_id == 0 ? "" : "<br/><br/> Topic
                                <span class='filter-text'>".concat(topic_name).concat("</span>")}
                                <!-- lesson type -->
                                ${param.lesson_type == 0 || param.lesson_type == null ? "" : "<br/><br/> Type
                                ".concat(param.lesson_type == 1 ? "<span class='filter-text'>Topic</span>" :
                                param.lesson_type == 2 ? "<span class='filter-text'>Lesson</span>" : "<span
                                    class='filter-text'>Quiz</span>")}
                                ${param.status == -1 || param.status == null ? "" : "<br/><br/> Status ".concat(param.status == 1
                                ? "<span class='filter-text'>Active</span>" : "<span
                                    class='filter-text'>Inactive</span>")}
                            </p>
                        </div>
                        <a style="text-decoration: none"
                            href="lesson-detail?action=add&subject_id=${subject.subject_id}">
                            <div class="data__header-add btn"><svg xmlns="http://www.w3.org/2000/svg" width="20"
                                    ...
                                    <path
                                        ...
                                        fill="white" style="stroke: rgb(255, 255, 255);"></path>
                                </svg>Add Lesson</div>
                        </a>
                    </div>
                        ...
                        <table id="data">
                            ...
                            <tbody>
                                <c:forEach var="lesson" items="${requestScope.lessons}">
                                    <tr>
                                        <td>
                                            <p class="${lesson.lesson_type != 1 ? " nested" : "bolded" }">
                                                <!-- show name but max 50 char only, then add ... -->
                                                <c:set var="lesson_name" value="${lesson.getName().length() > 50 ? lesson.getName().substring(0, 50).concat('...') : lesson.getName()}"></c:set>
                                                ${lesson.lesson_type != 1 ? lesson_name :
                                                    lesson_name.toUpperCase()}
                                            </p>
                                        </td>
                                        <td>
                                            <p class="row-item">${lesson.getOrder()}</p>
                                        </td>
                                        <td>
                                            <p class="row-item">${lesson.getLesson_type() == 1 ? "Topic" :
                                                lesson.getLesson_type() == 2 ? "Lesson" : "Quiz" }</p>
                                        </td>
                                        <td>
                                            <p class="status-column">${lesson.getStatus()}</p>
                                        </td>
                                        <td class="action-colum"><a
                                                href="lesson-detail?action=view&lesson_id=${lesson.lesson_id}">
                                                <div class="option"><img src="./Assets/Images/Icons/option.svg"></div>
                                            </a></td>
                                    </tr>
                                </c:forEach>
                            </tbody>
                    ...
                    <div class="paginition">
                        <div class="paginition-entry">
                            <P>${(requestScope.page-1) * 10+1}</P> - ${(requestScope.page) * 10 <= requestScope.size ?
                                (requestScope.page) * 10 :requestScope.size} of ${requestScope.size} </div>
                                <div class="paginition-actions">
                                    ...
                                    <div class="paginition-actions-select">
                                        <select id="pageSelect">
                                            <c:forEach varStatus="loop" begin="1" end="${requestScope.total_pages}">
                                                <c:if test="${requestScope.page == loop.index}">
                                                    <option selected>${loop.index}</option>
                                                </c:if>
                                                <c:if test="${requestScope.page != loop.index}">
            ...
            <script src="./Assets/Js/Expert/lessonList.js"></script>
