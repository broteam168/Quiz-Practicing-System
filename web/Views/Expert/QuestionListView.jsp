...
<html>
    <head>
        ...
        <script src="./Assets/Js/Expert/list.js"></script>
        <link rel="stylesheet" href="./Assets/Styles/Expert/questionlist.css"/>
        <title>Question Bank</title>
    </head>
    <body>
        <jsp:include page="./QuestionFilterPopup.jsp"></jsp:include> 
        <c:set scope="request" var="currentPage" value="Question List"></c:set>
        <jsp:include page="../Common/SideBarView.jsp"></jsp:include> 

            <main class="main">
                <h3 id="message">${requestScope.message}</h3>
            <div class="main__title">
                <div class="main__title-text">Question List</div>
                <div class="main__title-subtext">View and manage question</div>
            </div>
                            ...
                            <c:if test="${param.status != 2}"><div class="main__navigation-item" id="cancel"></c:if>
                                ...
                                <div class="main__data">
                                    <div class="data__header">
                                        <div class="data__header-title">${requestScope.size} Questions found</div> 
                                    <div   style="cursor: pointer;display: flex;gap:44px;"> <div onclick="alert('this function is developing...')" class="data__header-add" style="background: #1769C9"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                                            ...
                                            </svg>Import Question</div>
                                        <a style="text-decoration: none;" href="question-details?action=add"> <div class="data__header-add"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                                                <path d="M15 11H11V15H9V11H5V9H9V5H11V9H15M10 0C8.68678 0 7.38642 0.258658 6.17317 0.761205C4.95991 1.26375 3.85752 2.00035 2.92893 2.92893C1.05357 4.8043 0 7.34784 0 10C0 12.6522 1.05357 15.1957 2.92893 17.0711C3.85752 17.9997 4.95991 18.7362 6.17317 19.2388C7.38642 19.7413 8.68678 20 10 20C12.6522 20 15.1957 18.9464 17.0711 17.0711C18.9464 15.1957 20 12.6522 20 10C20 8.68678 19.7413 7.38642 19.2388 6.17317C18.7362 4.95991 17.9997 3.85752 17.0711 2.92893C16.1425 2.00035 15.0401 1.26375 13.8268 0.761205C12.6136 0.258658 11.3132 0 10 0Z" fill="white"/>
                                                </svg>Add Question</div></div></a>
                                </div>
                                <div class="data__filter">
                                    <div class="search-bar">
                                        ...
                                        <input id="myInputTextField" value="${param.content}"  placeholder="Search for content of question">
                                    </div>
                                    <div style="gap:10px;">
                                        <div class="data__filter-btn" id="data__filter-btnsearch" >
                                            Search   
                                        </div>
                                        <div class="data__filter-btn" id="data__filter-btnsearch2">
                                            Clear
                                        </div>
                                        ...
                                        <tbody>
                                            <c:forEach var="question" items="${requestScope.questions}">
                                                <tr>   
                                                    ...
                                                    <td class="content-col">${question.content}</td>
                                                    <td><div>Subject: ${question.subject}</div>
                                                        <div>Lesson: ${question.lesson}</div>
                                                        <div>Dimension ${question.dimension }</div>
                                                    </td>
                                                    <td>${question.level}</td>
                                                    <c:if test="${question.status == 0}">
                                                        <th><div class="status-table">Draft</div></th>
                                                        </c:if>
                                                        <c:if test="${question.status == 1}">
                                                        <th><div class="status-table-paid">Published</div></th>
                                                        </c:if>
                                                        <c:if test="${question.status == 2}">
                                                        <th><div class="status-table-cancel">Unpublished</div></th>
                                                        </c:if>
                                                    <th>
                                                        <div class="dropdown">
                                                            <div class="option dropbtn">
                                                                <img src="./Assets/Images/Icons/option.svg">
                                                            </div>
                                                            <div class="dropdown-content">
                                                                <c:if test="${question.status == 2}"><a href="question?action=show&question_id=${question.questionId}">Show</a>
                                                                </c:if>
                                                                <c:if test="${question.status == 1}">   <a href="question?action=hide&question_id=${question.questionId}">Hide</a>
                                                                </c:if>   <a href="question-details?action=edit&question_id=${question.questionId}">Edit</a>
                                                            </div>
                                                        </div>
